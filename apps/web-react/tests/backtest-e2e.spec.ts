...
test.describe('Backtest Module - Similar State Matching', () => {
  ...
  test('Frontend backtest page loads and renders', async ({ page }) => {
    ...
    // 4. Backtest results should auto-load from sentiment data (no button needed)
    // Wait for backtest results to appear
    await page.waitForSelector('text=/推荐信号回溯验证|暂无推荐信号回溯数据/', { timeout: 30000 });

    // 5. Check backtest results section
    const backtestSection = page.locator('text=/推荐信号回溯验证/');
    await expect(backtestSection).toBeVisible();

    // 6. Check for backtest result cards or empty state
    const hasBacktestData = await page.locator('text=/历史信号|最佳状态胜率|暂无足够历史数据/').count() > 0;
